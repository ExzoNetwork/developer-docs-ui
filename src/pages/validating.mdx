...
CUDA is required to make use of the GPU on your system. The provided Exzo Network release binaries are built on Ubuntu 20.04 with [CUDA Toolkit 10.1 update 1](https://developer.nvidia.com/cuda-toolkit-archive). If your machine is using a different CUDA version than you will need to rebuild from source.
...
```bash {{ title: '' }}
exzo-network config set --url https://testnet.exzoscan.io/rpc
```
...
```bash {{ title: '' }}
exzo-network transaction-count
```
...
```bash {{ title: '' }}
exzo-gossip spy --entrypoint bootstrap.testnet.exzoscan.io:8001
# Press ^C to exit
```
...
```bash {{ title: '' }}
exzo-network-keygen pubkey ~/validator-keypair.json
```
...
```bash {{ title: '' }}
exzo-network-keygen new --no-outfile
```

The corresponding identity public key can now be viewed by running:

```bash {{ title: '' }}
exzo-network-keygen pubkey ASK
```
...
1. Create the new validator identity keypair, Exzo Network-keygen new -o ~/new-validator-keypair.json.
2. Ensure that the new identity account has been funded, Exzo Network transfer ~/new-validator-keypair.json 500.
3. Run Exzo Network vote-update-validator ~/vote-account-keypair.json ~/new-validator-keypair.json ~/withdraw-authority.json to modify the validator identity in your vote account
...
1. Run Exzo Network epoch-info. If there is not much time remaining time in the current epoch, consider waiting for the next epoch to allow your validator plenty of time to restart and catch up.
2. Create the new vote authority keypair, Exzo Network-keygen new -o ~/new-vote-authority.json.
3. Determine the current vote authority keypair by running Exzo Network vote-account ~/vote-account-keypair.json. It may be validator's identity account (the default) or some other keypair. The following steps assume that ~/validator-keypair.json is that keypair.
4. Run Exzo Network vote-authorize-voter ~/vote-account-keypair.json ~/validator-keypair.json ~/new-vote-authority.json. The new vote authority is scheduled to become active starting at the next epoch.
5. Exzo Network-validator now needs to be restarted with the old and new vote authority keypairs, so that it can smoothly transition at the next epoch. Add the two arguments on restart: --authorized-voter ~/validator-keypair.json --authorized-voter ~/new-vote-authority.json
6. After the cluster reaches the next epoch, remove the --authorized-voter ~/validator-keypair.json argument and restart Exzo Network-validator, as the old vote authority keypair is no longer required.
...
```bash {{ title: '' }}
exzonetwork catchup ~/validator-keypair.json
```
...
```bash {{ title: '' }}
exzonetwork-keygen new -o ~/validator-stake-keypair.json
```
...
```bash {{ title: '' }}
exzonetwork create-stake-account ~/validator-stake-keypair.json 1
```

and then delegating that stake to your validator:

```bash {{ title: '' }}
exzonetwork delegate-stake ~/validator-stake-keypair.json ~/vote-account-keypair.json
```
...
```bash {{ title: '' }}
exzonetwork delegate-stake ~/validator-stake-keypair.json ~/some-other-vote-account-keypair.json
```
...
```bash {{ title: '' }}
exzonetwork deactivate-stake ~/validator-stake-keypair.json
```
...
```bash {{ title: '' }}
exzonetwork-gossip spy --entrypoint bootstrap.exzonetwork.com:8001
```
...
```bash {{ title: '' }}
exzonetwork vote-account ~/vote-account-keypair.json
```
...
```bash {{ title: '' }}
# Similar to exzonetwork-gossip, you should see your validator in the list of cluster nodes
...
```bash {{ title: '' }}
exzonetwork validator-info publish --keypair ~/validator-keypair.json <VALIDATOR_INFO_ARGS> <VALIDATOR_NAME>
```

For details about optional fields for VALIDATOR_INFO_ARGS:

```bash {{ title: '' }}
exzonetwork validator-info publish --help
```
...
```bash {{ title: '' }}
exzonetwork validator-info publish "Elvis Validator" -n elvis -w "https://elvis-validates.com"
```

Example query command:

```bash {{ title: '' }}
exzonetwork validator-info get
```
...
2. Add your validator identity pubkey to Keybase:
  ...
  - To check your pubkey, ensure you can successfully browse to `https://keybase.pub/<KEYBASE_USERNAME>/exzonetwork/validator-<PUBKEY>` {{className: 'break-words'}}
3. Add or update your `exzonetwork validator-info` with your Keybase username. The CLI will verify the `validator-<PUBKEY>` file
