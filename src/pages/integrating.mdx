...
export const sections = [
  { title: 'Add Exzo Network to Your Exchange', id: 'exzo_network_to_exchange' },
]
...
1. Install the Exzo Network command-line tool suite
2. Boot the node with at least the following parameters:

```bash {{ title: '' }}
exzonetwork-validator \
  --ledger <LEDGER_PATH> \
...
```bash {{ title: '' }}
exzonetwork transfer <USER_ADDRESS> <AMOUNT> --allow-unfunded-recipient --keypair <KEYPAIR> --url http://localhost:8899
```
...
```bash {{ title: '' }}
exzonetwork fees --url http://localhost:8899
```
...
```bash {{ title: '' }}
exzonetwork transfer <USER_ADDRESS> <AMOUNT> --allow-unfunded-recipient --no-wait --blockhash <RECENT_BLOCKHASH> --keypair <KEYPAIR> --url http://localhost:8899
```
